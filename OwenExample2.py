...
def writeRS(self, val):
    global rs, queueLogMsg
    rs = self.writeChar('r-S', val)
    pidOnOffButtonState(rs)
    if rs:
        queueLogMsg.put('Команда запуска PID. PID = {}'.format(rs))
    else:
        queueLogMsg.put('Команда остановки PID. PID = {}'.format(rs))
def writeSP(self, val):
    global sp, queueLogMsg
    sp = self.writeFloat24('SP', val)
    queueLogMsg.put('Изменение уставки SP = {:.2f} °C'.format(sp))

Owen.OwenDevice.writeRS = writeRS   # регистрируем новый метод класса Owen.OwenDevice
Owen.OwenDevice.writeSP = writeSP   # регистрируем новый метод класса Owen.OwenDevice
# Owen.OwenDevice.writeRS = classmethod(writeRS)
...
# Button.pidstate = pidstate   # регистрируем новый метод

pidOnOffButton = Button(topFrame, text="?", width=15, height=2, command=lambda: owenDev.writeRS(not rs))
...
spScale.bind("<ButtonRelease-1>", lambda e: (owenDev.writeSP(e.widget.get()), setattr(e.widget, 'lock', False)))
